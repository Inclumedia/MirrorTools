 ...
 *
 * ApiMirrorCreateUser
 * Created on 5 July 2014 by Nathan Larson
 *
...
class ApiMirrorCreateUser extends ApiBase {
	public function execute() {
		...
		$conds = array ( 'user_id' => $params['userid'] );
		$res = $dbw->selectRow( 'user', 'user_id', $conds );
		if ( $res ) {
			$this->dieUsage( 'User id ' . $params['userid'] . ' is already in the user table' );
		}
	...
	public function getAllowedParams() {
		return array(
			...
			'useremailtoken' => array(
				ApiBase::PARAM_TYPE => 'string',
				ApiBase::PARAM_DFLT => str_repeat( '0', 64 )
			),
